...
# Múi giờ cho reset theo ngày: "+07:00" (mặc định VN)
...
def _ensure():
    ...
    if not os.path.exists(USAGE_FILE):
        with open(USAGE_FILE, "w", encoding="utf-8") as f:
            json.dump({"uses": {}, "bonus": {}}, f, ensure_ascii=False)
...
def _today_str():
    # Lấy ngày theo offset UTC chỉ định (VD +07:00 cho VN)
...
def _bonus_entry(data, chat_id: str):
    bonus = data.setdefault("bonus", {})
    ent = bonus.get(chat_id)
    today = _today_str()
    if not ent or not isinstance(ent, dict) or ent.get("date") != today:
        ent = {"extra": 0, "date": today}
        bonus[chat_id] = ent
    return ent

def grant_bonus(chat_id: str, slots: int) -> int:
    with _LOCK:
        data = _load()
        ent = _bonus_entry(data, chat_id)
        ent["extra"] = int(ent.get("extra", 0)) + int(slots)
        _save(data)
        return ent["extra"]

def get_bonus(chat_id: str) -> int:
    data = _load()
    ent = _bonus_entry(data, chat_id)
    return int(ent.get("extra", 0))

def limit_for_today(chat_id: str) -> int:
    # FREE_USES + today's bonus
    return FREE_USES + get_bonus(chat_id)


def get_uses(chat_id: str) -> int:
    data = _load()
    ent = _get_entry(data, chat_id)
